...
namespace _Project.Scripts.Runtime.Gameplay.Presentation.Grid.Slot
{
    public class HexSlot : MonoBehaviour, ISlot, IPlacementTarget
    {
        ...
        private ISlotHighlight _highlight;

        [Header("Highlight Settings")] [SerializeField]
        private Color _highlightColor = new Color(1f, 1f, 0.5f, 1f);
        ...
        {
            MeshRenderer meshRenderer = GetComponent<MeshRenderer>() ?? GetComponentInChildren<MeshRenderer>();
            if (meshRenderer != null)
            {
                _highlight = new HexSlotHighlight(_highlightColor, _highlightIntensity);
                _highlight.Initialize(meshRenderer);
            }
        }
