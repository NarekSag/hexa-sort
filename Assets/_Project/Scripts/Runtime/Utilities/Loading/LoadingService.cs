...
namespace _Project.Scripts.Runtime.Utilities.Loading
{
    ...
    public sealed class LoadingService
    {
        ...
        {
            _watch?.Restart();
            CustomDebug.Log(LogCategory.Loading, $"{unit.GetType().Name} loading is started");
        }

        private async UniTask OnLoadingFinish(object unit, bool isError)
        {
            _watch?.Stop();
            CustomDebug.Log(LogCategory.Loading, $"{unit.GetType().Name} is {(isError ? "NOT " : "")}loaded with time {_watch?.ElapsedMilliseconds}ms");
            ...
            if (mainThreadId != currentThreadId) {
                _watch?.Restart();
                CustomDebug.Log(LogCategory.Loading, $"start switching from '{currentThreadId}' thread to main thread '{mainThreadId}'");
                await UniTask.SwitchToMainThread();
                _watch?.Stop();
                CustomDebug.Log(LogCategory.Loading, $"switch finished with time {_watch?.ElapsedMilliseconds}");
            }
        }
        ...
        public async UniTask BeginLoading(ILoadUnit loadUnit, bool skipExceptionThrow = false)
        {
            ...
            try {
                ...
                CustomDebug.LogError(LogCategory.Loading, e.Message, e);
